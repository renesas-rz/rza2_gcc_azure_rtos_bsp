...
static r_drp_bayer2grayscale_t param[R_DK2_TILE_NUM] __attribute__ ((section("UNCACHED_BSS")));
...
void sample_main(void)
{
    ...
    while (1)
    {
        ...
        for (tile_no = 0; tile_no < R_DK2_TILE_NUM; tile_no++)
        {
            ...
            param[tile_no].src = input_bufadr + ((R_BCD_CAMERA_WIDTH * (R_BCD_CAMERA_HEIGHT / R_DK2_TILE_NUM))
                                                                                                            * tile_no);
            param[tile_no].dst = output_bufadr + ((R_BCD_CAMERA_WIDTH * (R_BCD_CAMERA_HEIGHT / R_DK2_TILE_NUM))
                                                                                                            * tile_no);

            /* Set Image size */
            param[tile_no].width = R_BCD_CAMERA_WIDTH;
            param[tile_no].height = R_BCD_CAMERA_HEIGHT / R_DK2_TILE_NUM;
            ...
            /* Start DRP Library */
            /*********************/
            ret_val = R_DK2_Start(drp_lib_id[tile_no], (void *)&param[tile_no], sizeof(r_drp_bayer2grayscale_t));
            DRP_DRV_ASSERT(ret_val);
        }
