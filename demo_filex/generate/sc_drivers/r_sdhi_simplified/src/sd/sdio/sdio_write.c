...
******************************************************************************/
static int32_t _sdio_write_error(st_sdhndl_t *p_hndl, int32_t mode);
static int32_t _sdio_write_byte_error(st_sdhndl_t *p_hndl, int32_t mode);

/******************************************************************************
 * Function Name: sdio_write
 * Description  : write io register from (or at) the address (=adr) by the
 ...
 *                int32_t cnt      : number of write registers (byte)
 ...
 *                SD_ERR: end of error
 *****************************************************************************/
int32_t sdio_write(int32_t sd_port, uint8_t *buff, uint32_t func, uint32_t adr,
                    int32_t cnt, uint32_t op_code)
{
    ...
    if ((op_code & SD_IO_FORCE_BYTE ) == 0)
    {
        ...
        _sdio_write(p_hndl, buff, func, adr, cnt, op_code, blocklen);
    }
    else
    {
        /* ---- supply clock (data-transfer ratio) ---- */
        ...
        _sdio_write_byte(p_hndl, buff, func, adr, cnt, (op_code & SD_IO_INCREMENT_ADDR));
    }
 ...
 * Description  : write io register from specified address (=adr) by the
 *                number of bytes or blocks (=cnt)
 *                if SD Driver mode is SD_MODE_SW, data transfer by
 *                sddev_read_data function
 *                if SD Driver mode is SD_MODE_DMA, data transfer by DMAC
                        ...
                        uint32_t adr, int32_t cnt, uint32_t op_code, uint16_t blocklen)
{
    ...
    if (sec)    /* more than io block size (=blocklen bytes) */
    {
        ...
        for (i = sec; i > 0; i -= TRANS_BLOCKS)
        {
            ...
            {
                _sd_set_err(p_hndl, SD_ERR_NO_CARD);
                return _sdio_write_error(p_hndl, mode);
            }
            ...
            /* set argument */
            arg = ((func << 28) | 0x88000000u | (op_code << 26) | (adr << 9) | (uint32_t)sec);
            ...
            if (0 == mode)
            {
                ...
                trans_ret = _sdio_software_trans(p_hndl, buff, sec, SD_TRANS_WRITE, blocklen);
            }
            else
            {
                ...
                /* DMA data transfer */
                trans_ret = _sdio_dma_trans(p_hndl, sec, blocklen);
            }
    ...
    if (rem)    /* remaining bytes are less than io block size or not support multi block transfer */
    {
        ...
        /* set argument */
        arg = ((func << 28) | 0x80000000u | (op_code << 26) | (adr << 9) | (uint32_t)rem);

        /* ---- enable RespEnd and ILA ---- */
        _sd_set_int_mask(p_hndl, SD_INFO1_MASK_RESP, 0);

        /* issue CMD53 (IO_WRITE_EXTENDED_BYTE) */
        if (_sd_send_iocmd(p_hndl, CMD53_W_BYTE, arg) != SD_OK)
        {
            _sd_set_err(p_hndl, SD_ERR);
            return _sdio_write_error(p_hndl, mode);
        }
        ...
        /* software data transfer */
        trans_ret = _sdio_software_trans2(p_hndl, buff, rem, SD_TRANS_WRITE);
...
/******************************************************************************
 End of function _sdio_write
 *****************************************************************************/

/******************************************************************************
 * Function Name: _sdio_write_error
 * Description  : write io register space error
 * Arguments    : st_sdhndl_t *p_hndl  : SD handle
...
/******************************************************************************
 End of function _sdio_write_error
 *****************************************************************************/
                            ...
                            uint32_t adr, int32_t cnt, uint32_t op_code)
{
    ...
    if (sec)    /* more than io block size (=512 bytes) */
    {
        ...
        for (i = sec; i > 0; i--)
        {
            ...
            {
                _sd_set_err(p_hndl, SD_ERR_NO_CARD);
                return _sdio_write_byte_error(p_hndl, mode);
            }
            ...
            /* set argument */
            arg = ((func << 28) | 0x80000000u | (op_code << 26) | (adr << 9) | (uint32_t)0);

            /* ---- enable RespEnd and ILA ---- */
            _sd_set_int_mask(p_hndl, SD_INFO1_MASK_RESP, 0);

            /* issue CMD53 (IO_WRITE_EXTENDED_BLOCK) */
            _sd_send_iocmd(p_hndl, CMD53_W_BYTE, arg);
            ...
            if (0 == mode)
            {
                ...
                /* software data transfer */
                trans_ret = _sdio_software_trans(p_hndl, buff, 1, SD_TRANS_WRITE, 512);
            }
            else
            {
                /* disable card ins&rem interrupt for FIFO */
                ...
                /* DMA data transfer */
                trans_ret = _sdio_dma_trans(p_hndl, 1, 512);
            }
...
/******************************************************************************
 End of function _sdio_write_byte
 *****************************************************************************/

/******************************************************************************
 * Function Name: _sdio_write_byte_error
 * Description  : write io register space error
 * Arguments    : st_sdhndl_t *p_hndl  : SD handle
 *                int32_t mode  : data transfer mode
 *                  SD_MODE_SW  : software
...
/******************************************************************************
 End of function _sdio_write_byte_error
 *****************************************************************************/
