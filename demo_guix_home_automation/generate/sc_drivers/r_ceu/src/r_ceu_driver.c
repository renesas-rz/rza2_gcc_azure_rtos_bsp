 ...
 * @file         r_ceu_driver.c
 * @version      1.12
 * @brief        RZ/A2M CEU driver API functions
 ******************************************************************************/
...
static ceu_jpg_t  gs_local_jpg;
static ceu_dtif_t gs_local_dtif;
static uint32_t   gs_local_hwdth;
...
void R_CEU_Initialize(void (* const init_func)(uint32_t), const uint32_t user_num)
{
    ...
    /* init */
    gs_local_jpg   = CEU_IMAGE_CAPTURE_MODE;
    gs_local_dtif  = CEU_8BIT_DATA_PINS;
    gs_local_hwdth = 0;
...
ceu_error_t R_CEU_Open(const ceu_config_t * const config)
{
    ...
    if (CEU_OK == error)
    {
        gs_local_jpg  = config->jpg;
        gs_local_dtif = config->dtif;
        ...
        CEU.CAMCR.LONG = (uint32_t) ((((((((((CAMCR_FLDPOL_VALUE << CEU_SHIFT_VALUE_16) |

            /* Cast to an appropriate type */
            (uint32_t)(config->vdsel << 27u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->hdsel << 26u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->fldsel<< 25u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->dsel  << 24u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->dtif  << 12u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->dtary <<  8u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->jpg   <<  4u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->vdpol <<  1u)) |    /* Cast to an appropriate type */
            (uint32_t)(config->hdpol <<  0u));     /* Cast to an appropriate type */
        ...
        if (CEU_DATA_ENABLE_MODE != config->jpg)
        {
            ...
            if (NULL != config->cap)
            {
                gs_local_hwdth = config->cap->hwdth;
         ...
         */
        /* This implicit casting is valid because unsigned long is acceptable the value */
        CEU.CDOCR_A.LONG = (uint32_t) (((((CDOCR_CBE_VALUE << CEU_SHIFT_VALUE_16) |
            (CDOCR_CDS_VALUE << CEU_SHIFT_VALUE_4)) | /* Cast to an appropriate type */
            (uint32_t) (config->cols << CEU_SHIFT_VALUE_2)) | /* Cast to an appropriate type */
            (uint32_t) (config->cows << CEU_SHIFT_VALUE_1)) | /* Cast to an appropriate type */
            (uint32_t) (config->cobs) );
         ...
         * this register used in Data synchronous fetch mode.
         */
        CEU.CAIFR.LONG = (uint32_t) (((CAIFR_IFS_VALUE << CEU_SHIFT_VALUE_8)
            | (CAIFR_CIM_VALUE << CEU_SHIFT_VALUE_4)) |
        CAIFR_FC_VALUE);
         ...
         * other bit is reserved    : write value should always be 0.
         */
        CEU.CRCNTR.LONG = (uint32_t) (((CRCNTR_RVS_VALUE << CEU_SHIFT_VALUE_4)
            | (CRCNTR_RS_VALUE << CEU_SHIFT_VALUE_1)) |
        CRCNTR_RC_VALUE);
        ...
        CEU.CDBYR_A.LONG = 0u;    /* initialize */
        CEU.CDBCR_A.LONG = 0u;    /* initialize */
        CEU.CDAYR2_A.LONG = 0u;   /* initialize */
        CEU.CDACR2_A.LONG = 0u;   /* initialize */
        CEU.CDBYR2_A.LONG = 0u;   /* initialize */
        CEU.CDBCR2_A.LONG = 0u;   /* initialize */
 ...
 * @retval      Error codes of the CEU driver
 ******************************************************************************/
ceu_error_t R_CEU_Execute(const void * cayr, const void * cacr,
        const uint32_t chdw, ceu_onoff_t auto_capture)
{
    ceu_error_t error;
...
ceu_error_t R_CEU_Stop(void)
{
    ...
    /* Clear CE bit */
    CEU.CAPSR.LONG &= (uint32_t)~0x00000001;
 ...
 * @retval      Error codes of the CEU driver
 ******************************************************************************/
ceu_error_t R_CEU_Terminate(void (* const quit_func)(uint32_t),
        const uint32_t user_num)
{
    ...
    /* Wait for state change of CPKIL bit */
 ...
 * @retval      Error codes of the CEU driver
 ******************************************************************************/
void R_CEU_InterruptEnable(const ceu_int_type_t int_type, 
...
static ceu_error_t ceu_execute_check_prm(const void * cayr, const void * cacr, const uint32_t chdw)
{
    ...
    if (CEU_IMAGE_CAPTURE_MODE == gs_local_jpg)
    {
        /* Checking pointer variable */
    ...
    if (CEU_OK == error)
    {
        if (CEU_DATA_SYNC_MODE == gs_local_jpg)
        {
            /* Checking pointer variable */
            if (NULL == cayr)
            {
            ...
            if (CEU_16BIT_DATA_PINS == gs_local_dtif)
            {
                if ((gs_local_hwdth * 2) != chdw)
                {
                    error = CEU_ERR_PARAM;
                }
            }

            if (CEU_8BIT_DATA_PINS == gs_local_dtif)
            {
                if (gs_local_hwdth != chdw)
                {
                    error = CEU_ERR_PARAM;
                }
            }
    ...
    if (CEU_OK == error)
    {
        if (CEU_DATA_ENABLE_MODE == gs_local_jpg)
        {
            /* Checking pointer variable */
            if (NULL == cayr)
            {
