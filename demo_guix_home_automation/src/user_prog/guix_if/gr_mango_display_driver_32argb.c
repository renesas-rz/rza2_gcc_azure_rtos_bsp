...
void gr_mango_32argb_buffer_toggle(GX_CANVAS *canvas, GX_RECTANGLE *dirty)
{
    GX_RECTANGLE Limit;
    GX_RECTANGLE Copy;
...
int32_t gr_mango_32argb_lcd_init(void)
{
    vdc_error_t error;
    ...
    if (error == VDC_OK)
    {
        ...
        gr_disp_cnf.read_format = VDC_GR_FORMAT_ARGB8888;   /* Read Format ARGB8888 */
...
UINT gr_mango_graphics_driver_setup_32argb(GX_DISPLAY *display)
{
    _gx_display_driver_32argb_setup(display, GX_NULL, gr_mango_32argb_buffer_toggle);
    display->gx_display_handle = RZ_SCREEN_HANDLE;
    
    gr_mango_32argb_lcd_init();
