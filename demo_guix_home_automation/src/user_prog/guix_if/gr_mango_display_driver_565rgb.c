...
static USHORT video_buffer[(VIDEO_BUFFER_STRIDE * VIDEO_BUFFER_HEIGHT)] __attribute__ ((section("Video_RAM")));
...
void gr_mango_565rgb_buffer_toggle(GX_CANVAS *canvas, GX_RECTANGLE *dirty)
{
    GX_RECTANGLE Limit;
    GX_RECTANGLE Copy;
...
int32_t gr_mango_565rgb_lcd_init(void)
{
    vdc_error_t error;
    ...
    if (error == VDC_OK)
    {
        ...
        gr_disp_cnf.fb_stride = VIDEO_BUFFER_STRIDE * sizeof(USHORT);
                                                            /* Frame buffer stride             */
        ...
        gr_disp_cnf.read_format = VDC_GR_FORMAT_RGB565;     /* Read Format RGB565    */
        gr_disp_cnf.read_swap   = VDC_WR_RD_WRSWA_32_16BIT; /* Read Swap 32bit+16bit */
...
UINT gr_mango_graphics_driver_setup_565rgb(GX_DISPLAY *display)
{
    _gx_display_driver_565rgb_setup(display, GX_NULL, gr_mango_565rgb_buffer_toggle);
    display->gx_display_handle = RZ_SCREEN_HANDLE;
    
    gr_mango_565rgb_lcd_init();

    return GX_SUCCESS;
}

#endif /* (COLOR_FORMAT == GX_COLOR_FORMAT_565RGB) */
