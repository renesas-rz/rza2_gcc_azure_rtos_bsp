 ...
 * @file         r_ceu.h
 * @version      1.12
 * @brief        RZ/A2M CEU driver header
 ******************************************************************************/
...
/*! @enum ceu_edge_t
 *  @brief The edge for capturing the signal
 */
typedef enum
{
    CEU_EDGE_RISING = 0,
...
typedef struct
{
    uint32_t vofst; /*!< The capture start location in terms of the HD count
                      from a vertical sync signal (1-HD units) */
    uint32_t vwdth; /*!< The vertical capture period (4-HD units). */
    uint32_t hofst; /*!< The capture start location in terms of the number of clock cycles
                      from a horizontal sync signal (1-cycle units). */
...
typedef struct
{
    ceu_cap_rect_t * cap;    /*!< The horizontal/vertical timing
                                  of the Capture signals */
    ceu_clp_t      * clp;    /*!< Capture Filter Size
                                  (Set this param if Image capture mode.)*/
    ...
    ceu_edge_t       dsel;   /*!< Sets the edge for fetching the image data from an external module    */
    ceu_edge_t       fldsel; /*!< Sets the edge for capturing
                                  the field identification signal from an external module */
 ...
 * @retval      None
 ******************************************************************************/
void R_CEU_Initialize
            (void (* const init_func)(uint32_t), const uint32_t user_num);
 ...
 * @retval      Error codes of the CEU driver
 ******************************************************************************/
ceu_error_t R_CEU_Execute
            (const void * cayr, const void * cacr, const uint32_t chdw, ceu_onoff_t auto_capture);
 ...
 * @retval      Error codes of the CEU driver
 ******************************************************************************/
ceu_error_t R_CEU_Terminate
        (void (* const quit_func)(uint32_t), const uint32_t user_num);
