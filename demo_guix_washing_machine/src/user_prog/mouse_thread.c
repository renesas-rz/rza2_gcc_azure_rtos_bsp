...
void mouse_thread_entry(ULONG thread_input)
{
    ...
    mouse_event.gx_event_payload.gx_event_pointdata.gx_point_x = MAIN_DISPLAY_X_RESOLUTION/2;
    mouse_event.gx_event_payload.gx_event_pointdata.gx_point_y = MAIN_DISPLAY_Y_RESOLUTION/2;
        ...
        if(g_mouse!=UX_NULL)
        {
            ...
            if(rtn==UX_SUCCESS)
            {
                ...
                if(rtn==UX_SUCCESS)
                {
                    ...
                    else if(MAIN_DISPLAY_X_RESOLUTION<current_x)
                    {
                        current_x = MAIN_DISPLAY_X_RESOLUTION;
                    }
                    if(current_y<0)
                    {
                        current_y = 0;
                    }
                    else if(MAIN_DISPLAY_Y_RESOLUTION<current_y)
                    {
                        current_y = MAIN_DISPLAY_Y_RESOLUTION;
                    }
