...
@ INTC_GICC_IAR register address
    .EQU    INTC_GICC_IAR_ADDR,  0xE822200C
@ INTC_GICC_EOIR register address
    .EQU    INTC_GICC_EOIR_ADDR, 0xE8222010
@ INTC_GICD_ISACTIVER0 register address
    .EQU    INTC_GICD_ISACTIVER0_ADDR, 0xE8221300
@ INTC_GICD_IPRIORITYR0 register address
    .EQU    INTC_GICD_IPRIORITYR0_ADDR, 0xE8221400
@ INTC_GICC_HPPIR register address
    .EQU    INTC_GICC_HPPIR_ADDR, 0xE8222018
...
fiq_handler:
    SUB     lr, lr, #4
    SRSDB   sp!, #SYS_MODE              @;; Store LR_fiq and SPSR_fiq in system mode stack
    ...
    SUB     sp, sp, r1                  @;;
    PUSH    {r1, lr}
    BL      NMI_Handler_Interrupt       @;; Call NMI Handler
    POP     {r1, lr}
    ADD     sp, sp, r1
...
irq_handler:
    ...
    CPS     #SYS_MODE                   @;; Switch to system mode
    PUSH    {r0-r3, r12}                @;; Store other AAPCS registers
    LDR     r1, =INTC_GICC_HPPIR_ADDR
    LDR     r3, [r1]
    LDR     r2, =INTC_GICC_IAR_ADDR
    LDR     r0, [r2]                    @;; Read INTC_GICC_IAR_ADDR
    LDR     r2, =0x000003FF
    AND     r3, r0, r2
    LDR     r1, =0x000003FE
    CMP     r3, r1
...
int_active:
    ...
    SUB     sp, sp, r1                  @;;
    PUSH    {r1, lr}
    BL      INTC_Handler_Interrupt      @;; First argument(r0) = GICC_IAR_ADDR read value
    POP     {r1, lr}
    ADD     sp, sp, r1
    POP     {r0}
    LDR     r2, =INTC_GICC_EOIR_ADDR
    STR     r0, [r2]                    @;; Write GICC_EOIR
end_of_handler:
    POP     {r0-r3, r12}                @;; Restore registers
    RFEIA   sp!                         @;; Return from system mode stack using RFE

int_check:
    LDR     r2, =INTC_GICD_ISACTIVER0_ADDR
    LDR     r3, [r2]
    AND     r3, r3, #0x00000001
    CMP     r3, #0
    BNE     int_active
int_not_active:
    LDR     r2, =INTC_GICD_IPRIORITYR0_ADDR
    LDR     r3, [r2]
    STR     r3, [r2]
    B       end_of_handler
