 ...
 *********************************************************************************************************************/
ether_control_t g_eth_control_ch[ETHER_CHANNEL_MAX] =
{
    /* Ether = ch0, Phy access = ch0 */
    { &ETHERC0, &EDMAC0, &ETHERC0.PIR.LONG, 0, PORT_CONNECT_ET0 },

    /* Ether = ch1, Phy access = ch1 */
    { &ETHERC1, &EDMAC1, &ETHERC1.PIR.LONG, 0, PORT_CONNECT_ET1 },
};
...
static void ether_eint0 (uint32_t int_sense);
static void ether_eint1 (uint32_t int_sense);
...
static descriptor_t rx_descriptors[ETHER_CHANNEL_MAX][ETHER_CFG_EMAC_RX_DESCRIPTORS]
                                                    __attribute((section("UNCACHED_BSS"),aligned(32)));
 ...
 * g_ether_sc_table[0].config.ether_bufsize (the size of one buffer).
 * The g_ether_sc_table[0].config.ether_bufsize and EMAC_NUM_BUFFERS are defined
 * by macro in the file "r_ether_private.h".
...
#else
static etherbuffer_t ether_buffers[ETHER_CHANNEL_MAX] __attribute((section("_ETHERNET_BUFFERS"),aligned(32)));
#endif
...
void R_ETHER_Initial (void)
{
    ...
    for (i = 0; i < ETHER_CHANNEL_MAX; i++)
    {
        ...
        if (g_ether_sc_table[0].config.ether_use_linksta == 0)
        {
    ...
    g_eth_control_ch[0].phy_address = g_ether_sc_table[0].config.ether_ch0_phy_address;
    g_eth_control_ch[1].phy_address = g_ether_sc_table[0].config.ether_ch1_phy_address;
#elif ETHER_CHANNEL_0
    g_eth_control_ch[0].phy_address = g_ether_sc_table[0].config.ether_ch0_phy_address;
#else /* ETHER_CHANNEL_1 */
    g_eth_control_ch[0].phy_address = g_ether_sc_table[0].config.ether_ch1_phy_address;
#endif
...
ether_return_t R_ETHER_Open_ZC2 (uint32_t channel, const uint8_t mac_addr[], uint8_t pause)
{
    ...
    if ((g_ether_sc_table[0].config.ether_ch0_en == 1) && (g_ether_sc_table[0].config.ether_ch1_en == 0))
    {
        R_INTC_RegistIntFunc(INTC_ID_ETHER_EINT0, ether_eint0);
    }
    else if ((g_ether_sc_table[0].config.ether_ch0_en == 0) && (g_ether_sc_table[0].config.ether_ch1_en == 1))
    {
        R_INTC_RegistIntFunc(INTC_ID_ETHER_EINT1, ether_eint1);
    }
    else if (channel == 0)
    {
        R_INTC_RegistIntFunc(INTC_ID_ETHER_EINT0, ether_eint0);
    ...
    if (R_PHY_OK == phy_ret)
    {
        ...
        if (g_ether_sc_table[0].config.ether_use_linksta == 1)
        {
        ...
        if ((g_ether_sc_table[0].config.ether_ch0_en == 1) && (g_ether_sc_table[0].config.ether_ch1_en == 0))
        {
            R_INTC_SetPriority(INTC_ID_ETHER_EINT0, 26);
            R_INTC_Enable(INTC_ID_ETHER_EINT0);
        }
        else if ((g_ether_sc_table[0].config.ether_ch0_en == 0) && (g_ether_sc_table[0].config.ether_ch1_en == 1))
        {
            R_INTC_SetPriority(INTC_ID_ETHER_EINT1, 26);
...
ether_return_t R_ETHER_Close_ZC2 (uint32_t channel)
{
    ...
    if ((g_ether_sc_table[0].config.ether_ch0_en == 1) && (g_ether_sc_table[0].config.ether_ch1_en == 0))
    {
        R_INTC_Disable(INTC_ID_ETHER_EINT0);
    }
    else if ((g_ether_sc_table[0].config.ether_ch0_en == 0) && (g_ether_sc_table[0].config.ether_ch1_en == 1))
    {
...
int32_t R_ETHER_Read_ZC2 (uint32_t channel, void **pbuf)
{
    ...
    int32_t ret2;
    uint32_t channel_index = get_control_tbl_index(channel);
    descriptor_t * p_rx_desc = papp_rx_desc[channel_index];
...
#if (ETHER_CFG_USE_MMU)
    /* cast is needed to calculate virtural address from physical address */
    p_rx_desc = (descriptor_t *)(((uint32_t)p_rx_desc) + g_ether_sc_table[0].config.ether_mmu_uncached_diff);
#endif
    /* NULL type is (void*) */
    if (NULL == pbuf)
    {
        ...
        if (RACT != (p_rx_desc->status & RACT))
        {
            ...
            if (ETHER_ERR_MC_FRAME != ret)
            {
                ...
                else
                {
                     ...
                     */
#if (ETHER_CFG_USE_MMU)
                    /* R_MMU_PAtoVA(p_rx_desc->p_buf, pbuf); */
                    (*pbuf) = (void*)(((uint32_t) p_rx_desc->p_buf)
                            + g_ether_sc_table[0].config.ether_mmu_uncached_diff);
#else
                    (*pbuf) = (void *) p_rx_desc->p_buf;
#endif
                    /* Get bytes received */
...
int32_t R_ETHER_Read_ZC2_BufRelease (uint32_t channel)
{
    ...
    if (RACT != (p_rx_desc->status & RACT))
    {
        ...
        p_rx_desc->status &= (~status);
#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t)p_rx_desc->next, (uint32_t*)&papp_rx_desc[channel_index]);
#else
        papp_rx_desc[channel_index] = p_rx_desc->next;
#endif
    }
    pedmac_adr = g_eth_control_ch[channel_index].pedmac;
...
ether_return_t R_ETHER_Write_ZC2_GetBuf (uint32_t channel, void **pbuf, uint16_t *pbuf_size)
{
    /* Check argument */
    uint32_t channel_index = get_control_tbl_index(channel);
    descriptor_t* p_tx_desc = papp_tx_desc[channel_index];
...
#if (ETHER_CFG_USE_MMU)
    /* cast is needed to calculate virtural address from physical address */
    p_tx_desc = (descriptor_t*)(((uint32_t)p_tx_desc) + g_ether_sc_table[0].config.ether_mmu_uncached_diff);
#endif
    /* NULL type is (void*) */
    if (NULL == pbuf)
    {
...
#if (ETHER_CFG_USE_MMU)
    /*R_MMU_PAtoVA( papp_tx_desc[channel_index]->p_buf, pbuf);*/
    (*pbuf) = (void*)((p_tx_desc->p_buf) + g_ether_sc_table[0].config.ether_mmu_uncached_diff);
#else
    (*pbuf) = p_tx_desc->p_buf;
#endif
...
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t)p_tx_desc->next, (uint32_t*)&papp_tx_desc[channel_index]);
#else
...
void R_ETHER_LinkProcess (uint32_t channel)
{
    ...
    if (ETHER_FLAG_ON_LINK_ON == lchng_flag[channel_index])
    {
        ...
        else if (g_ether_sc_table[0].config.ether_use_linksta == 0)
        {
            /*
             * The status of the LINK signal became "link-up" even if PHY-LSI did not detect "link-up"
             * after a reset. To avoid this wrong detection, processing in R_ETHER_LinkProcess has been modified to
...
ether_return_t R_ETHER_WakeOnLAN (uint32_t channel)
{
    ...
    if (ETHER_SUCCESS == ret)
    {
        if (g_ether_sc_table[0].config.ether_use_linksta == 1)
        {
            ...
            if (g_ether_sc_table[0].config.ether_link_preset == petherc_adr->PSR.BIT.LMON)
            {
                ret = ETHER_SUCCESS;
            }
            else
...
int32_t R_ETHER_Read (uint32_t channel, void *pbuf)
{
    int32_t ret;
    int32_t ret2;
    uint8_t * p_read_buffer_address; /* Buffer location controlled by the Ethernet driver */
    ...
    ret = R_ETHER_Read_ZC2(channel, (void **) &p_read_buffer_address);
    ...
    if (ret > ETHER_NO_DATA)
    {
        ...
        memcpy(pbuf, p_read_buffer_address, (uint32_t )ret);
...
{
    ether_return_t ret;
    uint8_t * p_write_buffer_address;
    ...
    ret = R_ETHER_Write_ZC2_GetBuf(channel, (void **) &p_write_buffer_address, &write_buf_size);
    ...
    if (ETHER_SUCCESS == ret)
    {
        ...
        else
        {
            ...
            memcpy(p_write_buffer_address, pbuf, len);
...
static void ether_reset_mac (uint32_t channel)
{
    ...
    else if (ETHER_CHANNEL_0 == channel)
    {
        EDMAC0.EDMR.BIT.SWR = 1;
    }
    else
...
static void ether_init_descriptors (uint32_t channel)
{
    ...
    {
        /* calculate virtural address from physical address */
        p_descriptor = (descriptor_t *) &rx_descriptors[channel_index][i];

#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t)(&(ether_buffers[channel_index].buffer[i][0])),(uint32_t*)&address);

        /* copy address */
        p_descriptor->p_buf = (uint8_t *)address;
#else
        /* copy address */
        p_descriptor->p_buf = (uint8_t *) &(ether_buffers[channel_index].buffer[i][0]);
#endif

        p_descriptor->bufsize = ETHER_CFG_BUFSIZE;
        p_descriptor->size = 0;
        p_descriptor->status = RACT;
#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t) &rx_descriptors[channel_index][(i + 1)], (uint32_t*)&(p_descriptor->next));
#else
        p_descriptor->next = (descriptor_t *) &rx_descriptors[channel_index][(i + 1)];
#endif
    }

    /* The last descriptor points back to the start */
    p_descriptor->status |= RDLE;
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t) &rx_descriptors[channel_index][0], (uint32_t*)&(p_descriptor->next) );
#else
    p_descriptor->next = (descriptor_t *) &rx_descriptors[channel_index][0];
#endif
    /* Initialize application receive descriptor pointer */
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t)&rx_descriptors[channel_index][0], (uint32_t*)&papp_rx_desc[channel_index] );
#else
    ...
    {
        /* calc descripter address */
        p_descriptor = (descriptor_t *) &tx_descriptors[channel_index][i];

#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t)&(ether_buffers[channel_index].buffer[(ETHER_CFG_EMAC_RX_DESCRIPTORS + i)][0]),
...
#else
        /* copy address */
        p_descriptor->p_buf = (uint8_t *) &(ether_buffers[channel_index].buffer[(ETHER_CFG_EMAC_RX_DESCRIPTORS + i)][0]);
#endif

        p_descriptor->bufsize = 1; /* Set a value equal to or greater than 1. (reference to UMH)
...
#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t) &(tx_descriptors[channel_index][(i + 1)]), (uint32_t*)&p_descriptor->next);
#else
        p_descriptor->next = (descriptor_t *) &(tx_descriptors[channel_index][(i + 1)]);
#endif

    }

    /* The last descriptor points back to the start */
    p_descriptor->status |= TDLE;

#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t) &tx_descriptors[channel_index][0], (uint32_t*)&p_descriptor->next);
#else
    p_descriptor->next = (descriptor_t *) &tx_descriptors[channel_index][0];
#endif

    /* Initialize application transmit descriptor pointer */
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t) &tx_descriptors[channel_index][0], (uint32_t*)&papp_tx_desc[channel_index]);
#else
...
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t)papp_rx_desc[channel_index],(uint32_t*)&address);
    pedmac_adr->RDLAR.LONG = address;
#else
...
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t)papp_tx_desc[channel_index],(uint32_t*)&address);
    pedmac_adr->TDLAR.LONG = address;
#else
...
static void ether_configure_mac (uint32_t channel, const uint8_t mac_addr[], const uint8_t mode)
{
    ...
    mac_h = (((((uint32_t) mac_addr[0] << 24) | ((uint32_t) mac_addr[1] << 16))
            ...
            /* calc address */
            |  ((uint32_t) mac_addr[2] << 8)) | (uint32_t) mac_addr[3]);
...
static ether_return_t ether_power_on (ether_param_t const control)
{
    ...
    return power_on_control(channel);
...
static ether_return_t ether_set_multicastframe_filter (ether_param_t const control)
{
    ...
    channel = p_arg->channel;
    flag = p_arg->flag;
    channel_index = get_control_tbl_index(channel);
...
static uint8_t check_mpde_bit (void)
{
    if ( (g_ether_sc_table[0].config.ether_ch0_en + g_ether_sc_table[0].config.ether_ch1_en) == 1)
    {
        /* The MPDE bit can be referred to only when ETHERC operates. */
        if ( (ETHER_MODULE_USE == etherc_edmac_power_cont[0])
                && (((g_ether_sc_table[0].config.ether_ch0_en == 1)
