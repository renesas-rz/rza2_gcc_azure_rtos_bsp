...
static const r_gpio_port_pin_t s_phyrst_pin_list[] =
{
    GPIO_PORT_6_PIN_0,
};
...
int16_t phy_init (uint32_t ether_channel)
{
...
#if (1)
    /* Hardware Reset */
    st_r_drv_gpio_pin_list_t pin_rst;

    s_my_gpio_handle = open(DEVICE_INDENTIFIER "gpio", O_RDWR);
    pin_rst.p_pin_list = s_phyrst_pin_list;
    pin_rst.count = (sizeof(s_phyrst_pin_list)) / (sizeof(s_phyrst_pin_list[0]));
    direct_control(s_my_gpio_handle, CTL_GPIO_INIT_BY_PIN_LIST, &pin_rst);
    ...
    do
    {
        ...
        count++;
    } while ((reg & PHY_CONTROL_RESET) && (count < g_ether_sc_table[0].config.ether_phy_delay_reset));
...
#endif
    if (count < g_ether_sc_table[0].config.ether_phy_delay_reset)
    {
        /* PHY wait before access after reset(depends on PHY specification) */
        R_OS_TaskSleep(ETHER_CFG_PHY_WAIT);
...
static void phy_reg_read (uint32_t ether_channel, uint16_t *pdata)
{
    ...
    uint16_t reg_data;
    int16_t ret;
    volatile uint32_t * p_etherc_pir;

    ret = phy_get_pir_address(ether_channel, &p_etherc_pir);
    if (R_PHY_ERROR == ret)
    {
        return;
    }
    ...
    while (i > 0)
    {
        for (j = g_ether_sc_table[0].config.ether_phy_mii_wait; j > 0; j--)
        {
            (*p_etherc_pir) = 0x00000000;
        }

        for (j = g_ether_sc_table[0].config.ether_phy_mii_wait; j > 0; j--)
        {
            (*p_etherc_pir) = 0x00000001;
        }
        reg_data <<= 1;

        /* calc address */
        reg_data |= (uint16_t) (((*p_etherc_pir) & 0x00000008) >> 3); /* MDI read  */
...
static void phy_mii_write1 (uint32_t ether_channel)
{
    ...
    for (j = g_ether_sc_table[0].config.ether_phy_mii_wait; j > 0; j--)
    {
        (*p_etherc_pir) = 0x00000006;
    }

    for (j = g_ether_sc_table[0].config.ether_phy_mii_wait; j > 0; j--)
    {
        (*p_etherc_pir) = 0x00000007;
    }
...
static void phy_mii_write0 (uint32_t ether_channel)
{
    ...
    for (j = g_ether_sc_table[0].config.ether_phy_mii_wait; j > 0; j--)
    {
        (*p_etherc_pir) = 0x00000002;
    }

    for (j = g_ether_sc_table[0].config.ether_phy_mii_wait; j > 0; j--)
    {
        (*p_etherc_pir) = 0x00000003;
    }
...
static int16_t phy_get_pir_address (uint32_t ether_channel, volatile uint32_t ** pppir_addr)
{
    ...
    p_etherc_pir = g_eth_control_ch[ether_channel_index].p_reg_pir;

    (*pppir_addr) = p_etherc_pir;
