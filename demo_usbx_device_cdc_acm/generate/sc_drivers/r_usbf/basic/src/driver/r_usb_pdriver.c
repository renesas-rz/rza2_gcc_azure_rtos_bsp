 ...
 ******************************************************************************/
uint16_t g_usb_pstd_stall_pipe[USB_MAX_PIPE_NO + 1u] __attribute__ ((section ("USB_DATA_RAM")));   /* Stall Pipe info */
usb_cb_t g_usb_pstd_stall_cb __attribute__ ((section ("USB_DATA_RAM")));                           /* Stall Callback function */
uint16_t g_usb_pstd_config_num __attribute__ ((section ("USB_DATA_RAM_ZERO"))) = 0;                     /* Current configuration number */
uint16_t g_usb_pstd_alt_num[USB_ALT_NO] __attribute__ ((section ("USB_DATA_RAM")));                /* Alternate number */
uint16_t g_usb_pstd_remote_wakeup __attribute__ ((section ("USB_DATA_RAM_INIT"))) = USB_FALSE;          /* Remote wake up enable flag */
uint16_t g_usb_pstd_remote_wakeup_state;                /* Result for Remote wake up */

uint16_t g_usb_pstd_test_mode_select __attribute__ ((section ("USB_DATA_RAM")));                   /* Test mode selectors */
uint16_t g_usb_pstd_test_mode_flag __attribute__ ((section ("USB_DATA_RAM_INIT"))) = USB_FALSE;         /* Test mode flag */

uint16_t g_usb_pstd_eptbl_index[2][USB_MAX_EP_NO + 1u] __attribute__ ((section ("USB_DATA_RAM"))); /* Index of endpoint information table */
uint16_t g_usb_pstd_req_type __attribute__ ((section ("USB_DATA_RAM")));                           /* Request type */
uint16_t g_usb_pstd_req_value __attribute__ ((section ("USB_DATA_RAM")));                          /* Value */
uint16_t g_usb_pstd_req_index __attribute__ ((section ("USB_DATA_RAM")));                          /* Index */
uint16_t g_usb_pstd_req_length __attribute__ ((section ("USB_DATA_RAM")));                         /* Length */

uint16_t g_usb_pstd_pipe0_request __attribute__ ((section ("USB_DATA_RAM")));
uint16_t g_usb_peri_connected __attribute__ ((section ("USB_DATA_RAM")));                          /* Status for USB connect. */

/* Driver registration */
usb_pcdreg_t g_usb_pstd_driver __attribute__ ((section ("USB_DATA_RAM")));
usb_setup_t g_usb_pstd_req_reg __attribute__ ((section ("USB_DATA_RAM")));                         /* Device Request - Request structure */
...
void usb_pstd_pcd_task (void)
{
      ...
      while(1)
      {
        err = USB_TRCV_MSG(USB_PCD_MBX, (p_os_msg_t **)&p_mess, (usb_tm_t)1000);
#else  //(BSP_CFG_RTOS_USED == 1)

        err = USB_TRCV_MSG(USB_PCD_MBX, (p_os_msg_t **)&p_mess, (usb_tm_t)0);
...
usb_er_t usb_pstd_set_submitutr (usb_utr_t * utrmsg)
{
    ...
    {
        if (USB_NULL != g_p_usb_pstd_pipe[pipenum])   //B.
        {
            usb_cstd_pipe_msg_forward (utrmsg, pipenum); //E.
            return USB_OK;
        }
    ...
    else
    {
        ...
        {
            usb_cstd_pipe0_msg_clear (utrmsg, 0);  //D.
        }
        else
        {
            usb_cstd_pipe_msg_clear (utrmsg, pipenum);  //D.
        }
    }
...
usb_er_t usb_pstd_transfer_start(usb_utr_t * ptr)
{
    ...
    ptr->ip = USB_CFG_USE_USBIP; // USB_IP0;

//    p_tran_data = (usb_utr_t *)pvPortMalloc(sizeof(usb_utr_t));
    p_tran_data = (usb_utr_t *)R_OS_Malloc(sizeof(usb_utr_t),0);
    if (NULL == p_tran_data)
    {
        return USB_ERROR;
    }
...
//    printf("STACK:%d\r\n",uxTaskGetStackHighWaterMark((TaskHandle_t)g_pcd_tsk_hdl));
    err = USB_SND_MSG(USB_PCD_MBX, (p_os_msg_t* )p_tran_data);
    if (USB_OK != err)
    {
//        vPortFree (p_tran_data);
        R_OS_Free((void **) p_tran_data);
    }
...
usb_er_t usb_pstd_transfer_end(uint16_t pipe)
{
    ...
    else
    {
        ...
        /* Send message */
        return USB_SND_MSG(USB_PCD_MBX, (p_os_msg_t* )&utr);
    }
...
void usb_pstd_change_device_state(uint16_t state, uint16_t keyword, usb_cb_t complete)
{
    ...
    switch(state)
    {
        ...
        case USB_DO_REMOTEWAKEUP:
            ...
            /* Send message */
            err = USB_SND_MSG(USB_PCD_MBX, (p_os_msg_t* )&utr);
            if (USB_OK != err)
            {
