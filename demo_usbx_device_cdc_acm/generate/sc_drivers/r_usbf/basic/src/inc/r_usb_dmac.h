...
#define USB_DMA_CH_PRI             (25)     /* DMACmI interrupt priority level for USB Pipe : DxFIFO->buff */
#define USB_DMA_CH2_PRI            (25)     /* DMACmI interrupt priority level for USB Pipe : buff->DxFIFO */
...
/* Channel Configuration Register .b00:SEL bit set */
typedef enum e_dmaca_chcfg_fifo_select
{
    DMACA_CONFIG_SEL_D0FIFO = (0x0 << 0),
    DMACA_CONFIG_SEL_D1FIFO = (0x1 << 0)
} dmaca_chcfg_fifo_select_t;

/* Channel Configuration Register .b03:REQD bit set */
typedef enum e_dmaca_chcfg_dmareq_select
{
    DMACA_CONFIG_USB_SRC = (0x0 << 3),
    DMACA_CONFIG_USB_DES = (0x1 << 3)
} dmaca_chcfg_dmareq_select_t;

/* Channel Configuration Register .b20:SAD bit set */
typedef enum e_dmaca_chcfg_src_dir
{
    DMACA_CONFIG_SRC_ADDRESS_INC = (0x0 << 20),
    DMACA_CONFIG_SRC_ADDRESS_FIX = (0x1 << 20)
} dmaca_chcfg_src_dir_t;

/* Channel Configuration Register .b21:DAD bit set */
typedef enum e_dmaca_chcfg_des_dir
{
    DMACA_CONFIG_DES_ADDRESS_INC = (0x0 << 21),
    DMACA_CONFIG_DES_ADDRESS_FIX = (0x1 << 21)
} dmaca_chcfg_des_dir_t;

/* Channel Configuration Register .b24:DEM bit set */
typedef enum e_dmaca_chcfg_int_msk
{
    DMACA_CONFIG_DMAEND_INT_MASK_OFF = (0x0 << 24),
    DMACA_CONFIG_DMAEND_INT_MASK_ON = (0x1 << 24)
} dmaca_chcfg_int_msk_t;

/* Channel Configuration Register .b25:TCM bit set */
typedef enum e_dmaca_chcfg_tc_msk
{
    DMACA_CONFIG_DMATC_MASK_OFF = (0x0 << 25),
    DMACA_CONFIG_DMATC_MASK_ON = (0x1 << 25)
} dmaca_chcfg_tc_msk_t;

/* Channel Interval Register set */
typedef enum e_dmaca_ch_interval
{
    DMACA_INTERVAL_MIN = 0x0000,
    DMACA_INTERVAL_MAX = 0xffff
} dmaca_ch_interval_t;

/* DMA Control Register .b00:PR bit set */
typedef enum e_dmaca_dctrl_priority
{
    DMACA_DCTRL_PRIORITY_FIX = (0x0 << 0),
    DMACA_DCTRL_PRIORITY_ROUND = (0x1 << 0)
} dmaca_dctrl_priority_t;
...
//void r_dmaca_byteset_trans_enable(uint32_t buffer, uint8_t dma_ch);
