...
/* Check the setting values is valid.
 * Please review the setting values in r_ether_rza2_config.h if error message is output */
    #if !(ETHER_CFG_EMAC_RX_DESCRIPTORS >= 1)
        #error "ERROR- ETHER_CFG_EMAC_RX_DESCRIPTORS - \
Transmission descriptors is out of range defined in r_ether_rza2_config.h."
    #endif

    #if !(ETHER_CFG_EMAC_TX_DESCRIPTORS >= 1)
        #error "ERROR- ETHER_CFG_EMAC_TX_DESCRIPTORS - \
Receive descriptors is out of range defined in r_ether_rza2_config.h."
    #endif

    #if !((ETHER_CFG_BUFSIZE % 32) == 0)
        #error "ERROR- ETHER_CFG_BUFSIZE - \
transmission and receive buffers is not 32-byte aligned in r_ether_rza2_config.h."
    #endif
 ...
 *********************************************************************************************************************/
extern ether_control_t g_eth_control_ch[];
...
/**********************************************************************************************************************
 * @fn            ether_set_phy_mode
 * @brief         Set PHY mode.
 * @warning       None.
 * @param [in]    uint8_t connect: Channel Number.
 * @param [out]   None.
 * @retval        None.
 *********************************************************************************************************************/
